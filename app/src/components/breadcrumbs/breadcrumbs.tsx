...
export const Breadcrumbs = ({ initialCategory }: { initialCategory: Category }) => {
    ...
    useEffect(() => {
        if (params) {
            ...
            setCategoryPath([level0, level1, level2, level3].filter(Boolean) as Category[]);
        } else {
            setCategoryPath([]);
        }
    ...
    const handleBreadcrumbClick = (category: Category) => {
        ...
        if (category.level === 0) {
            ...
            const level1Category = categoryPath.find(cat => cat.level === 1);
            const level2Category = categoryPath.find(cat => cat.level === 2);
            pathSegments = [initialCategory?.slug, level1Category?.slug, level2Category?.slug, category.slug];
        }
        ...
        <Breadcrumb>
            <BreadcrumbList>
                <BreadcrumbItem className="text-[0.6rem] md:text-xs">
                    <BreadcrumbLink href="/">
                        <Home className="h-3 lg:h-4 w-3 lg:w-4" />
                    </BreadcrumbLink>
                </BreadcrumbItem>
