...
const ProductSheet: React.FC = () => {
    ...
    const {isOpen, openSheet, closeSheet, variantId, source} = useProductSheet();
    const [product, setProduct] = useState<Product | null>(null);
    const [merchants, setMerchants] = useState<Merchant[]>([]);
    const [otherProducts, setOtherProducts] = useState<Product[]>([]);
    const [currentImage, setCurrentImage] = useState(0);
    const [loading, setLoading] = useState(true);
    const [activateAnimation, setActivateAnimation] = useState(null);
    ...
    useEffect(() => {
        const fetchProductData = async () => {
            ...
            try {
                const filters = {
                    filter: `variants.variantId = ${variantId}`,
                };
                ...
                if (productData.length > 0) {
                    ...
                    console.error('No product found for variantId:', variantId);
                }
            } catch (error) {
    ...
    // Function to handle sheet close and remove variantId from the URL
    const handleSheetClose = () => {
        closeSheet();
    };

    // Function to handle sheet open via button click
                                ...
                                <div className="grid md:grid-cols-2 gap-8 max-w-full mx-auto px-4 py-8 ">
                                    ...
                                    <div>
                                        ...
                                        <Link href={variant.merchantLink}>
                                            <Button className="w-auto mb-4 text-white bg-blue-500 hover:bg-blue-600">
                                                Zum Shop
                                            </Button>
                                        </Link>
                                                ...
                                                <Table className="table-fixed w-full">
                                                    ...
                                                    <TableBody>
                                                        {sameEanVariants.map((v) => {
                                                            ...
                                                            return (
                                                                <TableRow onClick={() => router.push(v.merchantLink)} key={v.variantId} className="cursor-pointer">
                                                                    <TableCell className="w-1/6 md:w-auto table-cell">
                                                                        {merchant?.logo_image?.data?.attributes?.url && (
                                                                            <Image
                                                                                ...
                                                                                width={50}
                                                                                alt={merchant?.name ?? 'merchat-logo'}
                                                                                height={25}
                                                                    ...
                                                                    <TableCell className="flex justify-end items-center md:table-cell">
                                                                        <Link href={v.merchantLink}>
                                                                            <Button className="bg-blue-500 text-white hover:bg-blue-600" size="sm" variant="outline">Zum Angebot</Button>
                                                                        </Link>
                                                                    </TableCell>
                                                                </TableRow>
                                                            );
