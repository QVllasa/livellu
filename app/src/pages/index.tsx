...
const CategoryTree = ({ categories }) => {
    const renderCategory = (category) => {
        ...
        const children = category.child_categories?.data?.filter(child => child.attributes.identifier.startsWith(`${level + 1}_`));
        ...
        return (
            <AccordionItem key={category.identifier} value={category.identifier}>
                ...
                {children?.length > 0 && (
                    <AccordionContent>
                        <ul>
                            {children.map((child) => renderCategory(child.attributes))}
                        </ul>
                    </AccordionContent>
                )}
            </AccordionItem>
